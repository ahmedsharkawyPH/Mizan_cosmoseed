...
import { PurchaseInvoice, Supplier, Product, PurchaseItem } from '../types';
import SearchableSelect from './SearchableSelect';
import { 
  Search, Calendar, Truck, ShoppingBag, TrendingUp, 
  ChevronLeft, Package, FileText, X, ArrowUpRight, 
  Receipt, History, Info, Filter, ListChecks, BarChart3,
  CreditCard
} from 'lucide-react';

interface SupplierReportCompProps {
...
export default function SupplierReportComp({ 
  initialSupplierId = '', 
  initialProductId = '',
  startDate = '',
  endDate = ''
}: SupplierReportCompProps) {
                      ...
                      <table className="w-full text-xs text-right">
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {selectedInvoice.items.map((item: any, idx: number) => {
                            ...
                            return (
                              <tr key={idx}>
                                <td className="px-4 py-3 font-bold text-slate-700">{products.find(p => p.id === item.product_id)?.name || 'صنف غير معروف'}</td>
