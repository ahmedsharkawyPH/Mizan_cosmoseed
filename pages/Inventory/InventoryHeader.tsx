...
const InventoryHeader: React.FC<Props> = ({ onExportPdf, onExportExcel, onAddNew, onSyncPrices }) => (
    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <h1 className="text-3xl font-black text-slate-800 flex items-center gap-3">
            <Package className="w-8 h-8 text-blue-600" /> {t('stock.title')}
        </h1>
        <div className="flex flex-wrap gap-2">
            <button onClick={onSyncPrices} className="bg-slate-100 text-slate-600 px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-sm hover:bg-slate-200 transition-all active:scale-95 border border-slate-200">
                <PlusCircle className="w-5 h-5 rotate-45" /> تحديث الأسعار
            </button>
