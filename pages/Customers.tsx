...
export default function Customers() {
  ...
  // Distribution Line Management State
  ...
  const statementData = useMemo(() => {
    ...
    invoices.forEach(inv => {
      items.push({ date: inv.date, description: `Invoice #${inv.invoice_number}`, debit: inv.net_total, credit: 0, rawDate: new Date(inv.date) });
    });
    const payments = db.getCashTransactions().filter(tx => tx.category === 'CUSTOMER_PAYMENT' && tx.reference_id === statementCustomer.id);
    payments.forEach(pay => {
       items.push({ date: pay.date, description: `Payment: ${pay.notes || '-'}`, debit: 0, credit: pay.amount, rawDate: new Date(pay.date) });
    });
    ...
    items.forEach(item => {
        ...
        finalStatement.push({ date: item.date, description: item.description, debit: item.debit, credit: item.credit, balance: balance });
    });
  ...
  const handleStatementExportPDF = async () => {
        ...
        try {
            ...
            } else { pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight); }
    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-gray-200 overflow-x-auto">
          <button 
            ...
            className={`px-6 py-3 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'LIST' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
          ...
          <button 
            ...
            className={`px-6 py-3 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'ANALYSIS' ? 'border-purple-600 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
              <BarChart3 className="w-4 h-4" />
              {t('cust.tab_analysis')}
          </button>
          <button 
            onClick={() => setActiveTab('DIST_LINES')}
            className={`px-6 py-3 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'DIST_LINES' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
                    ...
                    {customers.filter(c => c.name.toLowerCase().includes(search.toLowerCase())).map(c => {
                        ...
                        <tr key={c.id} className="border-b hover:bg-gray-50 group">
                            ...
                            <td className={`p-4 text-right rtl:text-left font-bold ${c.current_balance > 0 ? 'text-red-500' : 'text-green-500'}`}>
                                ...
                                {c.credit_limit && c.credit_limit > 0 && ( <div className="text-[10px] text-gray-400 font-normal">Limit: {c.credit_limit}</div> )}
                            </td>
                            <td className="p-4 text-center">
                                <div className="flex justify-center gap-2">
                                    <button onClick={() => setStatementCustomer(c)} className="text-blue-600 hover:bg-blue-50 px-3 py-1 rounded text-xs font-medium border border-blue-200" title={t('common.statement')} >
                                        <FileText className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => openEditModal(c)} className="text-gray-500 hover:text-blue-600 hover:bg-gray-100 p-1.5 rounded transition-colors" title={t('list.edit')} >
                                        <Edit className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleDelete(c.id)} className="text-gray-500 hover:text-red-600 hover:bg-red-50 p-1.5 rounded transition-colors" title="Delete" >
              ...
              <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left rtl:text-right min-w-[800px]">
                      <thead className="bg-slate-50 text-slate-600 uppercase text-xs">
                          <tr>
                              <th className="p-4 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('name')}><div className="flex items-center gap-1">{t('cust.name')} <ArrowUpDown className="w-3 h-3 text-slate-400" /></div></th>
                              <th className="p-4 cursor-pointer hover:bg-slate-100 text-right rtl:text-left" onClick={() => handleSort('totalSales')}><div className="flex items-center gap-1 justify-end rtl:justify-start">{t('cust.total_sales')} <ArrowUpDown className="w-3 h-3 text-slate-400" /></div></th>
                              <th className="p-4 cursor-pointer hover:bg-slate-100 text-right rtl:text-left" onClick={() => handleSort('totalPaid')}><div className="flex items-center gap-1 justify-end rtl:justify-start">{t('cust.total_paid')} <ArrowUpDown className="w-3 h-3 text-slate-400" /></div></th>
                              <th className="p-4 cursor-pointer hover:bg-slate-100 text-center" onClick={() => handleSort('repaymentRatio')}><div className="flex items-center gap-1 justify-center">{t('cust.repayment_ratio')} <ArrowUpDown className="w-3 h-3 text-slate-400" /></div></th>
                              <th className="p-4 cursor-pointer hover:bg-slate-100 text-center" onClick={() => handleSort('invCount')}><div className="flex items-center gap-1 justify-center">{t('cust.inv_count')} <ArrowUpDown className="w-3 h-3 text-slate-400" /></div></th>
                              <th className="p-4 cursor-pointer hover:bg-slate-100 text-center" onClick={() => handleSort('monthlyRate')}><div className="flex items-center gap-1 justify-center">{t('cust.monthly_rate')} <ArrowUpDown className="w-3 h-3 text-slate-400" /></div></th>
                          </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100">
                          {sortedAnalytics.map(c => (
                              <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                                  ...
                                  <td className="p-4 text-center">
                                      <div className="flex items-center justify-center gap-2">
                                          <div className="w-16 h-2 bg-gray-100 rounded-full overflow-hidden">
                                              <div className={`h-full rounded-full ${c.repaymentRatio >= 80 ? 'bg-emerald-500' : c.repaymentRatio >= 50 ? 'bg-amber-500' : 'bg-red-500'}`} style={{width: `${Math.min(c.repaymentRatio, 100)}%`}}></div>
                                          </div>
                                  ...
                                  <td className="p-4 text-center"><span className="bg-purple-50 text-purple-700 px-2 py-1 rounded-full text-xs font-bold border border-purple-100">{c.monthlyRate.toFixed(1)} /mo</span></td>
                              </tr>
                          ))}
                      </tbody>
                  </table>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                  ...
                  <div className="flex gap-3 mb-6">
                      <input 
                        ...
                        placeholder="اسم خط التوزيع الجديد..."
                        value={newLineName}
                      ...
                      <button 
                        ...
                        className="bg-emerald-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-emerald-700 transition-colors shadow-sm flex items-center gap-2"
                      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-1 md:col-span-2">
                <label className="block text-xs text-gray-500 mb-1 flex items-center gap-1"><MapPin className="w-3 h-3" /> {t('cust.address')}</label>
                <input placeholder={t('cust.address')} className="w-full border p-2 rounded" value={form.address} onChange={e => setForm({...form, address: e.target.value})} />
            </div>
            <div className="col-span-1 md:col-span-2">
                <label className="block text-xs text-gray-500 mb-1 flex items-center gap-1"><Truck className="w-3 h-3" /> {t('cust.dist_line')}</label>
                <select 
