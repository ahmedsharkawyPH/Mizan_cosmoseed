  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      try {
        ...
        const data = await readExcelFile<any>(file);
        let count = 0;
        
        // Process sequentially to avoid ID collision
        for (const row of data) {
            // Basic validation: Check for Name and Code
            // Support multiple column naming conventions (English/Arabic/Lowercase)
            const name = row.name || row.Name || row['اسم الصنف'] || row['الاسم'];
            const code = row.code || row.Code || row['كود'] || row['الكود'];
            ...
            const productData = {
                code: String(code),
                name: String(name),
                package_type: row.package_type || row['نوع العبوة'] || undefined,
                items_per_package: Number(row.items_per_package || row['العدد'] || 1)
            };
            
            const batchData = {
                quantity: Number(row.quantity || row.qty || row['الكمية'] || row['رصيد'] || 0),
                purchase_price: Number(row.purchase_price || row.cost || row['تكلفة'] || row['سعر الشراء'] || 0),
                selling_price: Number(row.selling_price || row.price || row['سعر'] || row['سعر البيع'] || 0),
                batch_number: row.batch_number || row.batch || 'IMPORT',
                expiry_date: row.expiry_date || '2099-12-31'
            };

            await db.addProduct(productData, batchData);
            count++;
            
            // Small delay to ensure unique IDs if generated by timestamp
            await new Promise(resolve => setTimeout(resolve, 5)); 
        }
        
        setProducts(db.getProductsWithBatches());
        alert(`${t('common.success_import')}: ${count} items`);
