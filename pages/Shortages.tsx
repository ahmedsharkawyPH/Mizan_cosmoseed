  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2"><AlertTriangle className="w-8 h-8 text-red-600" />{t('nav.shortages')}</h1>
                ...
                <tbody className="divide-y divide-gray-100">
                    {currentList.map((item: any, idx: number) => (
                        <tr key={idx} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 text-center font-mono">{item.lastPrices[0] ? `${currency}${item.lastPrices[0]}` : '-'}</td>
                            <td className="px-6 py-4 text-center font-medium">{item.monthlyAvg}</td>
                            ...
                            <td className="px-6 py-4 text-center">{item.stock <= 0 ? <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-bold">Out Stock</span> : item.stock < item.monthlyAvg ? <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-xs font-bold">Low Stock</span> : <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold">OK</span>}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
