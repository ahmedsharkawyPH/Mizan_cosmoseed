...
import { Invoice, CashTransaction, Customer, Product, CartItem } from '../types';
import SearchableSelect from '../components/SearchableSelect';
import { 
  Search, Calendar, User, Hash, ShoppingBag, TrendingUp, 
...
export default function CustomerReport() {
  ...
  const totals = useMemo(() => {
    ...
    const totalPayments = filteredPayments.reduce((sum, tx) => {
      // In CUSTOMER_PAYMENT, RECEIPT means customer paid us, EXPENSE means we paid them (refund)
                          ...
                          {filteredPayments.map((tx) => (
                            <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="px-6 py-4 text-xs text-slate-400 font-bold hidden sm:table-cell">
                                {tx.reference_id && tx.reference_id !== selectedCustomerId ? `فاتورة #${db.getInvoices().find(i => i.id === tx.reference_id)?.invoice_number || '---'}` : 'حساب عام'}
                              </td>
                            </tr>
                          ))}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        <table className="w-full text-xs text-right">
                          ...
                          <tbody className="divide-y divide-slate-50">
                            {selectedInvoice.items.map((item: CartItem, idx: number) => {
                              const pId = item.product.id;
                              const isTarget = pId === selectedProductId;
                              const price = item.unit_price || 0;
                              const discount = item.discount_percentage || 0;
                      ...
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2">
                        <div className="flex justify-between text-xs font-bold text-slate-500">
                          ...
                          <span>{currency}{selectedInvoice.total_before_discount.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-xs font-bold text-rose-500">
                          <span>إجمالي الخصم:</span>
                          <span>{currency}{(selectedInvoice.total_discount + (selectedInvoice.additional_discount || 0) + (selectedInvoice.cash_discount_value || 0)).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-sm font-black text-slate-900 border-t border-slate-200 pt-2">
                          <span>الصافي النهائي:</span>
                          <span>{currency}{selectedInvoice.net_total.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                  </div>
